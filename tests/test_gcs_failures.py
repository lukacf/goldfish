...
class TestGCSUploadFailures:
    ...
    def test_gcs_upload_failure_cleanup(self, temp_dir):
        ...
        tracker.update_job_status(
            "job-test-001",
            "failed",
            error="GCS upload failed: ServiceException: 401 Unauthorized"
        )
    ...
    def test_gcs_upload_partial_failure_consistency(self, temp_dir):
        ...
        assert job_0["artifact_uri"] == gcs_uri_0, \
            "Successful upload should set artifact_uri"

        # Job 1 should NOT have artifact_uri (failed upload)
        job_1 = db.get_job("job-test-001")
        assert job_1["artifact_uri"] is None or job_1["artifact_uri"] == "", \
            "Failed upload should not set artifact_uri"
