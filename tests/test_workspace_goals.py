...
class TestWorkspaceManagerGoals:
    ...
    def test_create_workspace_persists_goal(self, temp_dir):
        ...
        try:
            create_fn = server.create_workspace.fn if hasattr(server.create_workspace, 'fn') else server.create_workspace
            create_fn(
                name="new-ws",
            ...
            mock_db.set_workspace_goal.assert_called_once_with(
                "new-ws", "Implement new feature X"
            )
        finally:
            server.reset_server()
...
class TestGetWorkspaceGoalTool:
    ...
    def test_returns_goal_for_workspace(self, temp_dir):
        ...
        try:
            get_goal_fn = server.get_workspace_goal.fn if hasattr(server.get_workspace_goal, 'fn') else server.get_workspace_goal
...
class TestUpdateWorkspaceGoalTool:
    ...
    def test_updates_workspace_goal(self, temp_dir):
        ...
        try:
            update_goal_fn = server.update_workspace_goal.fn if hasattr(server.update_workspace_goal, 'fn') else server.update_workspace_goal
