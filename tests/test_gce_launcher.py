...
def test_launch_with_capacity_search(
    mock_build_startup, mock_resource_launcher_class, launcher
):
...
@patch("goldfish.infra.gce_launcher.build_startup_script")
def test_launch_simple_no_gpu(
    mock_build_startup, mock_run_gcloud, mock_tempfile, launcher
):
...
@patch("goldfish.infra.gce_launcher.run_gcloud")
@patch("goldfish.infra.gce_launcher.build_startup_script")
def test_launch_simple_with_gpu(
    mock_build_startup, mock_run_gcloud, mock_tempfile, launcher
):
...
def test_create_disk_basic(mock_run_gcloud, launcher):
    ...
    launcher.create_disk(
        disk_name="test-disk", zone="us-central1-a", size_gb=200, disk_type="pd-ssd"
    )
...
def test_snapshot_disk(mock_run_gcloud, launcher):
    ...
    launcher.snapshot_disk(
        disk_name="my-disk", snapshot_name="my-snapshot", zone="us-central1-a"
    )
...
def test_get_instance_status_not_found(mock_run_gcloud, launcher):
    ...
    mock_run_gcloud.return_value = Mock(
        returncode=1, stdout="", stderr="Instance not found"
    )
...
def test_get_instance_logs_fallback_serial(mock_run_gcloud, mock_popen, launcher):
    ...
    mock_run_gcloud.return_value = Mock(
        returncode=0, stdout="Serial console output\n", stderr=""
    )
...
def test_wait_for_termination_success(
    mock_sleep, mock_time, mock_subprocess, mock_run_gcloud, launcher
):
...
@patch("goldfish.infra.gce_launcher.run_gcloud")
@patch("goldfish.infra.gce_launcher.time.time")
def test_wait_for_termination_timeout(mock_time, mock_run_gcloud, launcher):
...
@patch("goldfish.infra.gce_launcher.ResourceLauncher")
@patch("goldfish.infra.gce_launcher.build_startup_script")
def test_launch_filters_gpu_resources(
    mock_build_startup, mock_resource_launcher_class, launcher
):
