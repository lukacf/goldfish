...
# =============================================================================
# Output Syncer Tests - Preemption protection for checkpoints
# =============================================================================


class TestOutputSyncer:
    """Tests for output_syncer_section - periodic checkpoint sync to GCS."""

    def test_output_syncer_section_generates_background_process(self):
        """Output syncer should create bash function that runs in background."""
        script = output_syncer_section(bucket="test-bucket", bucket_path="runs/test-run")

        # Should define start_output_syncer function
        assert "start_output_syncer()" in script or "start_output_syncer ()" in script

        # Should run in background with &
        assert "&" in script

        # Should track PID for cleanup
        assert "OUTPUT_SYNCER_PID" in script

    def test_output_syncer_uses_correct_gcs_paths(self):
        """Output syncer should upload to correct gs://bucket/path/outputs/ location."""
        script = output_syncer_section(bucket="my-bucket", bucket_path="runs/stage-abc123")

        # Should target the outputs GCS path
        assert "gs://my-bucket/runs/stage-abc123/outputs" in script

        # Should use gsutil rsync for incremental uploads
        assert "gsutil" in script
        assert "rsync" in script

    def test_output_syncer_respects_sync_interval(self):
        """Output syncer should sleep for configured interval between syncs."""
        script = output_syncer_section(bucket="test-bucket", bucket_path="runs/test", sync_interval=120)

        # Should have the configured sleep interval
        assert "120" in script or "OUTPUT_SYNC_INTERVAL=120" in script

    def test_output_syncer_default_interval(self):
        """Output syncer should use 60 second default interval."""
        script = output_syncer_section(bucket="test-bucket", bucket_path="runs/test")

        # Default should be 60 seconds
        assert "60" in script

    def test_output_syncer_loops_while_docker_running(self):
        """Output syncer should continue until Docker process exits."""
        script = output_syncer_section(bucket="test-bucket", bucket_path="runs/test")
    ...
    def test_output_syncer_final_sync_after_docker_exits(self):
        """Output syncer should do final sync after Docker exits."""
        script = output_syncer_section(bucket="test-bucket", bucket_path="runs/test")

        # Should have a final sync after Docker exits
        # Look for comment or duplicate rsync
        assert "Final" in script or script.count("rsync") >= 2

    def test_output_syncer_syncs_from_mnt_outputs(self):
        """Output syncer should sync from /mnt/outputs/ directory."""
        script = output_syncer_section(bucket="test-bucket", bucket_path="runs/test")

        # Should reference /mnt/outputs
        assert "/mnt/outputs" in script

    def test_output_syncer_excludes_goldfish_metadata(self):
        """Output syncer should exclude .goldfish/ internal files."""
        script = output_syncer_section(bucket="test-bucket", bucket_path="runs/test")

        # Should exclude .goldfish directory
        assert ".goldfish" in script
        assert "-x" in script  # rsync exclude flag

    def test_output_syncer_excludes_temp_files(self):
        """Output syncer should exclude temp and partial files."""
        script = output_syncer_section(bucket="test-bucket", bucket_path="runs/test")

        # Should exclude temp/partial files that may be mid-write
        assert ".tmp" in script or ".part" in script

    def test_output_syncer_ignores_upload_errors(self):
        """Output syncer should not fail if gsutil rsync fails."""
        script = output_syncer_section(bucket="test-bucket", bucket_path="runs/test")
...
class TestBuildStartupScriptWithOutputSyncer:
    """Tests for build_startup_script with output syncer integration."""

    def test_build_startup_script_starts_output_syncer(self):
        """Startup script should start output syncer after Docker begins."""
        script = build_startup_script(
            bucket="test-bucket",
            bucket_prefix="",
            run_path="runs/test",
        ...
        # Should call start_output_syncer
        assert "start_output_syncer" in script

        # Output syncer should start AFTER Docker run begins
        import re

        docker_run_idx = script.find("DOCKER_PID=$!")
        calls = list(re.finditer(r"^start_output_syncer$", script, re.MULTILINE))
        assert len(calls) > 0, "start_output_syncer should be called"
        syncer_call_idx = calls[0].start()
    ...
    def test_build_startup_script_output_syncer_uses_correct_bucket_path(self):
        """Output syncer should use the correct GCS bucket path from build_startup_script."""
        script = build_startup_script(
            bucket="my-ml-bucket",
            bucket_prefix="goldfish",
            run_path="runs/stage-xyz789",
            image="test-image",
            entrypoint="/bin/bash",
            env_map={},
        )

        # Should include correct output path
        assert "gs://my-ml-bucket/goldfish/runs/stage-xyz789/outputs" in script
