...
class TestFromRefValidation:
    ...
    def test_validate_from_ref_accepts_slashed_local_branch(self) -> None:
        """feature/foo, release/v1 are valid local branch names."""
        from goldfish.validation import validate_from_ref

        validate_from_ref("feature/foo")  # Should not raise
        validate_from_ref("release/v1")  # Should not raise

    def test_validate_from_ref_rejects_explicit_remote_refs(self) -> None:
        import pytest
