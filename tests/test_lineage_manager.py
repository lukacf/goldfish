...
class TestWorkspaceLineage:
    ...
    def test_get_workspace_lineage_with_parent(self, test_db):
        ...
        test_db.create_workspace_lineage(
            ...
            parent_version="v2",
            description="Branched from parent"
        )
...
class TestRunProvenance:
    ...
    def test_get_run_provenance_complete(self, test_db):
        ...
        test_db.add_signal(
            ...
            signal_name="tokens",
            signal_type="npy",
            storage_location="gs://bucket/tokens"
        )
...
class TestBranchWorkspace:
    ...
    def test_branch_workspace_creates_branch(self, test_db):
        ...
        result = lineage_mgr.branch_workspace(
            from_workspace="main_ws",
            from_version="v2",
            new_workspace="experiment_ws",
            reason="Testing new approach"
        )

        # Verify git branch was created
        workspace_manager.branch_workspace.assert_called_once_with(
            "main_ws", "v2", "experiment_ws"
        )
