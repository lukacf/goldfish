...
def pipeline_project():
    ...
    try:
        ...
        subprocess.run(
            ["git", "config", "user.email", "test@example.com"], cwd=dev_repo, capture_output=True, check=True
        )
        subprocess.run(["git", "config", "user.name", "Test User"], cwd=dev_repo, capture_output=True, check=True)

        # Create main branch with initial commit
        (dev_repo / "README.md").write_text("# Test Project")
        subprocess.run(["git", "add", "-A"], cwd=dev_repo, capture_output=True, check=True)
        subprocess.run(["git", "commit", "-m", "Initial commit"], cwd=dev_repo, capture_output=True, check=True)
...
class TestPipelineExecutionWorkflow:
    ...
    def test_full_pipeline_execution_workflow(self, pipeline_project):
        ...
        mount_result = workspace_manager.mount(
            workspace="simple-ml-pipeline", slot="w1", reason="Setting up pipeline for E2E test"
        )
...
"""
        ...
        checkpoint_result = workspace_manager.checkpoint(
            slot="w1", message="Initial pipeline setup with preprocess and train stages"
        )
        ...
        with db._conn() as conn:
            stage_run_record = conn.execute(
                "SELECT * FROM stage_runs WHERE id = ?", (stage_run.stage_run_id,)
            ).fetchone()
        ...
        with db._conn() as conn:
            versions = conn.execute(
                "SELECT * FROM workspace_versions WHERE workspace_name = ?", ("simple-ml-pipeline",)
            ).fetchall()
    ...
    def test_pipeline_with_invalid_stage_fails(self, pipeline_project):
        ...
        workspace_manager.create_workspace(
            name="invalid-stage-test", goal="Test error handling", reason="Testing invalid stage execution"
        )
        workspace_manager.mount(workspace="invalid-stage-test", slot="w1", reason="Testing invalid stage")
...
"""
        ...
        with pytest.raises(GoldfishError, match="Stage.*not found"):
            stage_executor.run_stage(
                workspace="invalid-stage-test", stage_name="nonexistent_stage", reason="This should fail"
            )
    ...
    def test_pipeline_validation_catches_missing_files(self, pipeline_project):
        ...
        # Create workspace
        workspace_manager.create_workspace(
            name="validation-test", goal="Test pipeline validation", reason="Testing pipeline validation logic"
        )
        workspace_manager.mount(workspace="validation-test", slot="w1", reason="Setting up validation test")
