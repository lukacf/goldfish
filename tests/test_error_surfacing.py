...
class TestStateMdWriteErrors:
    ...
    def test_write_failure_is_logged(self, temp_dir, caplog):
        ...
        assert any("STATE.md" in record.message or "write" in record.message.lower()
    ...
    def test_mkdir_failure_is_logged(self, temp_dir, caplog):
        ...
        # Should have logged a warning
        assert any("directory" in record.message.lower() or "mkdir" in record.message.lower()
                   for record in caplog.records), \
    ...
    def test_temp_file_cleanup_failure_is_logged(self, temp_dir, caplog):
        ...
        assert any("temp" in record.message.lower() or "clean" in record.message.lower()
                   for record in caplog.records), \
...
class TestGitMetadataErrors:
    ...
    def test_branch_info_failure_includes_warning(self, temp_dir, caplog):
        ...
        assert any("metadata" in record.message.lower() or "git" in record.message.lower()
                   for record in caplog.records), \
    ...
    def test_snapshot_info_failure_logs_warning(self, temp_dir, caplog):
        ...
        assert any("snapshot" in record.message.lower()
                   for record in caplog.records), \
    ...
    def test_list_snapshots_failure_logs_warning(self, temp_dir, caplog):
        ...
        assert result == []
        # Should log warning
        assert any("snapshot" in record.message.lower() or "list" in record.message.lower()
                   for record in caplog.records), \
...
""")
        ...
        assert ("line" in error_msg.lower() or
                "yaml" in error_msg.lower() or
                "slots" in error_msg.lower()), \
...
""")
        ...
        assert "project_name" in error_msg, \
            f"Expected field name in error: {error_msg}"
...
class TestDatabaseErrors:
    ...
    def test_db_init_error_includes_path(self, temp_dir):
        ...
        assert "blocking" in error_msg or str(blocking_file) in error_msg, \
            f"Expected path in error: {error_msg}"
    ...
    def test_job_not_found_includes_job_id(self, temp_dir):
        ...
        error_msg = str(exc_info.value)
        assert "job-that-doesnt-exist-12345" in error_msg, \
            f"Expected job ID in error: {error_msg}"
