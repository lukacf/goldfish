...
class TestListSourcesFiltering:
    ...
    def test_list_sources_combined_filters_and_pagination(self, temp_dir):
        ...
        page1 = db.list_sources(
            status="available", created_by="external", limit=10, offset=0
        )
        assert len(page1) == 10
        ...
        page2 = db.list_sources(
            status="available", created_by="external", limit=10, offset=10
        )
        assert len(page2) == 10
